...
contract('DebtMarket', (accounts) => {
    ...
    it('should get loans issued to a borrower', async () => {
        ...
        assert.equal(loans[0].toNumber(), 1000, "Debt amount should be 1000");
        assert.equal(loans[1].toNumber(), rate, "Rate should be 10");
        assert.equal(loans[2].toNumber(), dueDate, "Due date didn't match");
        assert.equal(new Date(loans[3].toNumber() * 1000).toDateString(), new Date().toDateString(), "Incorrect issue date");
        assert.equal(loans[4].toNumber(), BORROWER, "Party should be BORROWER");
...
contract('Borrower', (accounts) => {
    it('should borrow the loan', async () => {
        const lenderInstance = await Lender.deployed();
        const debtMarketInstance = await DebtMarket.deployed();
        const ARRANGER = 2;
        ...
        await debtMarketInstance.borrow(borrower, nftTokenId, debtAmount);

        const borrowRepayDetail = await debtMarketInstance.getBorrowRepayDetail(borrower, nftTokenId);
        assert.equal(borrowRepayDetail[0].toNumber(), debtAmount, "Borrowed amount is not matched with the debt amount");
    });
    ...
    it('should repay the loan', async () => {
        ...
        let borrowRepayDetail = await debtMarketInstance.getBorrowRepayDetail(borrower, nftTokenId);
        assert.equal(borrowRepayDetail[0].toNumber(), debtAmount, "Borrowed amount is not matched with the debt amount");

        await debtMarketInstance.repay(borrower, nftTokenId, debtAmount);

        borrowRepayDetail = await debtMarketInstance.getBorrowRepayDetail(borrower, nftTokenId);
        assert.equal(borrowRepayDetail[0].toNumber(), debtAmount, "Borrowed amount is not matched with the debt amount");
        assert.equal(borrowRepayDetail[1].toNumber(), debtAmount, "Borrowed amount is not repaid");
    });
